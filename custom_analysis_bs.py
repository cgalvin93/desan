#time python ~/design_and_analysis/custom_analysis_bs.py index
...
def plot_hist(dist,xlabel,title):
    ...
    plt.close()
#
startdir='/wynton/home/kortemme/cgalvin/metab_targets/clean'
ligand_dirs=[i for i in os.listdir(startdir) if os.path.isdir(os.path.join(startdir,i))==True]
#
for ligand_dir in ligand_dirs:  ################################################################################################################################################
    ...
    try:
        ...
        print('problem processing '+str(ligand_dir))
        pdf.close()
        continue
    #
...
okay for this what i need to do is separate jobs for each different target to isolate where its going wrong
